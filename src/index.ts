...
async function run(): Promise<void> {
  try {
    ...
    try {
      ...
      execSync(
        `git commit -m "Update ${path.basename(formulaFile, ".rb")} to version ${releaseTag}"`,
        { cwd: tempDir },
      );
...
function parseChecksums(checksumsContent: string): PlatformChecksums {
  ...
  if (
    ...
    throw new Error(
      "Could not parse all required checksums from checksums.txt",
    );
  }
...
function updatePlatformBlock(
  ...
  if (conditionStartIndex === -1) {
    throw new Error(
      `Could not find condition: ${condition} in block starting with: ${blockStart}`,
    );
  }
  ...
  return (
    content.substring(0, conditionStartIndex) +
    updatedBlock +
